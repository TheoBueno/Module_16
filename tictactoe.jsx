const Board = () => {
  ...
  if (status === 'Player O Wins'){  
    setTimeout(() => {alert('Player O Wins')}, 300);
  } else if (status === 'Player X Wins') {
      setTimeout(() => {alert('Player X Wins')}, 300);}

  // Part 1 step 1 code goes here
  // Use conditional logic to set a variable to either 'Player O' or  'Player X'
  let turn = `Next Player: X`
  if (player === 1){
    turn = `Next Player: X`
  } else {
    turn = `Next Player: O`
  ...
  return (
    <>
      <div className="game-board">
        ...
        <div id="info">
          <h1 id='turn'>{turn}</h1><h1> {status}</h1>
        </div>
      </div>
    </>
  );
...
const Square = ({ takeTurn, id }) => {
  ...
  return (
    <button
      ...
      className={tik == '1' ? 'red' : tik == '0' ? 'white' : 'blank'}
      onClick={() => {
        setTik(takeTurn(id));
        setFilled(true);
